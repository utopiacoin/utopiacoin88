...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars UTOPIACOIND and UTOPIACOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

UTOPIACOIND=${UTOPIACOIND:-${1}/utopiacoind}
CLI=${UTOPIACOINCLI:-${1}/utopiacoin-cli}
...
$UTOPIACOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$UTOPIACOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
