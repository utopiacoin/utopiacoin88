...
*Setup instructions for a gitian build of Utopiacoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Utopiacoin
...
- [Building Utopiacoin](#building-utopiacoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-utopiacoin-host/README.md).
...
Clone the git repositories for utopiacoin and gitian and then checkout the utopiacoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the utopiacoin repository
...
To build Utopiacoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the utopiacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/utopiacoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/utopiacoin/utopiacoin
...
For example:
```bash
URL=https://github.com/laanwj/utopiacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit utopiacoin=${COMMIT} --url utopiacoin=${URL} ../utopiacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit utopiacoin=${COMMIT} --url utopiacoin=${URL} ../utopiacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit utopiacoin=${COMMIT} --url utopiacoin=${URL} ../utopiacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/utopiacoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/utopiacoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/utopiacoin-build.assert
```
...
[utopiacoin/gitian.sigs](https://github.com/utopiacoin/gitian.sigs/) repository, or if that's not possible create a pull
