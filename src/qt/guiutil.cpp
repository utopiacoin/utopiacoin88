...
#include "utopiacoinaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setFont(utopiacoinAddressFont());
#if QT_VERSION >= 0x040700
    // We don't want translators to use own addresses in translations
    // and this is the only place, where this address is supplied.
    widget->setPlaceholderText(QObject::tr("Enter a Utopiacoin address (e.g. %1)").arg("1NS17iag9jJgTHD1VXjvLCEnZuQ3rJDE9L"));
#endif
...
bool parseUtopiacoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!UtopiacoinUnits::parse(UtopiacoinUnits::UCN, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
QString formatUtopiacoinURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(UtopiacoinUnits::format(UtopiacoinUnits::UCN, info.amount, false, UtopiacoinUnits::separatorNever));
        paramCount++;
    }
...
{
    CTxDestination dest = CUtopiacoinAddress(address.toStdString()).Get();
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        ...
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, NULL, NULL, utopiacoinAppUrl, NULL, NULL);
    }
